...
pub mod mixture_server {
    ...
    #[async_trait]
    pub trait Mixture: Send + Sync + 'static {
        ///
    ...
    ///
    #[derive(Debug)]
    pub struct MixtureServer<T: Mixture> {
        inner: Arc<T>,
    ...
    impl<T: Mixture> MixtureServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: Mixture> Clone for MixtureServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Mixture> tonic::server::NamedService for MixtureServer<T> {
        const NAME: &'static str = "bilibili.app.show.mixture.v1.Mixture";
    }
}
