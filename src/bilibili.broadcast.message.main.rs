...
impl LikeState {
    ...
    pub fn as_str_name(&self) -> &'static str {
        match self {
            LikeState::Default => "DEFAULT",
            LikeState::Like => "LIKE",
            LikeState::Dislike => "DISLIKE",
        }
    }
...
pub mod native_page_server {
    ...
    #[async_trait]
    pub trait NativePage: Send + Sync + 'static {
        ///
    ...
    ///
    #[derive(Debug)]
    pub struct NativePageServer<T: NativePage> {
        inner: Arc<T>,
    ...
    impl<T: NativePage> NativePageServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: NativePage> Clone for NativePageServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: NativePage> tonic::server::NamedService for NativePageServer<T> {
        const NAME: &'static str = "bilibili.broadcast.message.main.NativePage";
    }
}
/// Generated server implementations.
pub mod resource_server {
    ...
    impl<T: Resource> tonic::server::NamedService for ResourceServer<T> {
        const NAME: &'static str = "bilibili.broadcast.message.main.Resource";
    }
}
/// Generated server implementations.
pub mod search_server {
    ...
    impl<T: Search> tonic::server::NamedService for SearchServer<T> {
        const NAME: &'static str = "bilibili.broadcast.message.main.Search";
    }
}
