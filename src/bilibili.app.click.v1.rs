...
pub mod click_server {
    ...
    #[async_trait]
    pub trait Click: Send + Sync + 'static {
        ///
        async fn heart_beat(
            &self,
    ...
    ///
    #[derive(Debug)]
    pub struct ClickServer<T: Click> {
        inner: Arc<T>,
    ...
    impl<T: Click> Clone for ClickServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Click> tonic::server::NamedService for ClickServer<T> {
        const NAME: &'static str = "bilibili.app.click.v1.Click";
    }
}
