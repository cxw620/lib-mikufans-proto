...
pub mod vega_deneb_rpc_server {
    ...
    #[async_trait]
    pub trait VegaDenebRpc: Send + Sync + 'static {
        ///
    ...
    ///
    #[derive(Debug)]
    pub struct VegaDenebRpcServer<T: VegaDenebRpc> {
        inner: Arc<T>,
    ...
    impl<T: VegaDenebRpc> VegaDenebRpcServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: VegaDenebRpc> Clone for VegaDenebRpcServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: VegaDenebRpc> tonic::server::NamedService for VegaDenebRpcServer<T> {
        const NAME: &'static str = "bilibili.vega.deneb.v1.VegaDenebRPC";
    }
}
