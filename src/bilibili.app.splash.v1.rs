...
pub mod splash_server {
    ...
    #[async_trait]
    pub trait Splash: Send + Sync + 'static {
        ///
        async fn list(
            &self,
    ...
    ///
    #[derive(Debug)]
    pub struct SplashServer<T: Splash> {
        inner: Arc<T>,
    ...
    impl<T: Splash> Clone for SplashServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Splash> tonic::server::NamedService for SplashServer<T> {
        const NAME: &'static str = "bilibili.app.splash.v1.Splash";
    }
}
