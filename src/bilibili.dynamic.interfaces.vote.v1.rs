...
pub mod vote_server {
    ...
    #[async_trait]
    pub trait Vote: Send + Sync + 'static {
        ///
    ...
    impl<T: Vote> Clone for VoteServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Vote> tonic::server::NamedService for VoteServer<T> {
        const NAME: &'static str = "bilibili.dynamic.interfaces.vote.v1.Vote";
    }
}
