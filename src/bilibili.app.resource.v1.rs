...
pub mod legacy_resource_server {
    ...
    #[async_trait]
    pub trait LegacyResource: Send + Sync + 'static {
        ///
        async fn download(
            &self,
    ...
    ///
    #[derive(Debug)]
    pub struct LegacyResourceServer<T: LegacyResource> {
        inner: Arc<T>,
    ...
    impl<T: LegacyResource> LegacyResourceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: LegacyResource> Clone for LegacyResourceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: LegacyResource> tonic::server::NamedService for LegacyResourceServer<T> {
        const NAME: &'static str = "bilibili.app.resource.v1.LegacyResource";
    }
}
/// Generated server implementations.
pub mod module_server {
    ...
    impl<T: Module> tonic::server::NamedService for ModuleServer<T> {
        const NAME: &'static str = "bilibili.app.resource.v1.Module";
    }
}
