...
impl Act {
    ...
    pub fn as_str_name(&self) -> &'static str {
        match self {
            Act::Na => "ACT_NA",
            Act::AddFollowing => "ACT_ADD_FOLLOWING",
            Act::DelFollowing => "ACT_DEL_FOLLOWING",
            Act::AddWhisper => "ACT_ADD_WHISPER",
            Act::DelWhisper => "ACT_DEL_WHISPER",
            Act::AddBlack => "ACT_ADD_BLACK",
            Act::DelBlack => "ACT_DEL_BLACK",
            Act::DelFollower => "ACT_DEL_FOLLOWER",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
...
pub mod relation_interface_server {
    ...
    #[async_trait]
    pub trait RelationInterface: Send + Sync + 'static {
        ///
        async fn at_search(
            &self,
    ...
    ///
    #[derive(Debug)]
    pub struct RelationInterfaceServer<T: RelationInterface> {
        inner: Arc<T>,
    ...
    impl<T: RelationInterface> RelationInterfaceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: RelationInterface> Clone for RelationInterfaceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: RelationInterface> tonic::server::NamedService
    for RelationInterfaceServer<T> {
        const NAME: &'static str = "bilibili.relation.interfaces.RelationInterface";
    }
}
