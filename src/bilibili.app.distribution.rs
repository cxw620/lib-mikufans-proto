...
pub mod distribution_server {
    ...
    ///
    #[derive(Debug)]
    pub struct DistributionServer<T: Distribution> {
        inner: Arc<T>,
    ...
    impl<T: Distribution> DistributionServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: Distribution> Clone for DistributionServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Distribution> tonic::server::NamedService for DistributionServer<T> {
        const NAME: &'static str = "bilibili.app.distribution.Distribution";
    }
}
