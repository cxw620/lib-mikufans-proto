...
pub mod probe_server {
    ...
    ///
    #[derive(Debug)]
    pub struct ProbeServer<T: Probe> {
        inner: Arc<T>,
    ...
    impl<T: Probe> Clone for ProbeServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Probe> tonic::server::NamedService for ProbeServer<T> {
        const NAME: &'static str = "bilibili.api.probe.v1.Probe";
    }
}
/// Generated server implementations.
pub mod probe_service_server {
    ...
    #[async_trait]
    pub trait ProbeService: Send + Sync + 'static {
        ///
        async fn echo(
            &self,
    ...
    ///
    #[derive(Debug)]
    pub struct ProbeServiceServer<T: ProbeService> {
        inner: Arc<T>,
        accept_compression_encodings: EnabledCompressionEncodings,
        send_compression_encodings: EnabledCompressionEncodings,
        max_decoding_message_size: Option<usize>,
    ...
    impl<T: ProbeService> ProbeServiceServer<T> {
        pub fn new(inner: T) -> Self {
            Self::from_arc(Arc::new(inner))
        }
        pub fn from_arc(inner: Arc<T>) -> Self {
    ...
    impl<T: ProbeService> Clone for ProbeServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: ProbeService> tonic::server::NamedService for ProbeServiceServer<T> {
        const NAME: &'static str = "bilibili.api.probe.v1.ProbeService";
    }
}
