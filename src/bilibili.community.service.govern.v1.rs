...
pub mod qoe_server {
    ...
    #[async_trait]
    pub trait Qoe: Send + Sync + 'static {
        ///
        async fn qoe_report(
            &self,
    ...
    impl<T: Qoe> Clone for QoeServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Qoe> tonic::server::NamedService for QoeServer<T> {
        const NAME: &'static str = "bilibili.community.service.govern.v1.Qoe";
    }
}
