...
pub mod popular_server {
    ...
    #[async_trait]
    pub trait Popular: Send + Sync + 'static {
        ///
        async fn index(
            &self,
    ...
    ///
    #[derive(Debug)]
    pub struct PopularServer<T: Popular> {
        inner: Arc<T>,
    ...
    impl<T: Popular> PopularServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: Popular> Clone for PopularServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Popular> tonic::server::NamedService for PopularServer<T> {
        const NAME: &'static str = "bilibili.app.show.popular.v1.Popular";
    }
}
