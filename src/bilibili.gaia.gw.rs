...
pub mod gaia_server {
    ...
    #[async_trait]
    pub trait Gaia: Send + Sync + 'static {
        ///
    ...
    impl<T: Gaia> Clone for GaiaServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Gaia> tonic::server::NamedService for GaiaServer<T> {
        const NAME: &'static str = "bilibili.gaia.gw.Gaia";
    }
}
