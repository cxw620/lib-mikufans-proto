...
pub mod anti_harassment_service_server {
    ...
    #[async_trait]
    pub trait AntiHarassmentService: Send + Sync + 'static {
        ///
    ...
    ///
    #[derive(Debug)]
    pub struct AntiHarassmentServiceServer<T: AntiHarassmentService> {
        inner: Arc<T>,
    ...
    impl<T: AntiHarassmentService> AntiHarassmentServiceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: AntiHarassmentService> Clone for AntiHarassmentServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: AntiHarassmentService> tonic::server::NamedService
    for AntiHarassmentServiceServer<T> {
        const NAME: &'static str = "bilibili.polymer.community.govern.v1.AntiHarassmentService";
    }
}
