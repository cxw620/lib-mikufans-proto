...
pub mod tv_server {
    ...
    impl<T: Tv> Clone for TvServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Tv> tonic::server::NamedService for TvServer<T> {
        const NAME: &'static str = "bilibili.broadcast.message.tv.Tv";
    }
}
