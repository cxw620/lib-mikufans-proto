...
impl MsgType {
    ...
    pub fn as_str_name(&self) -> &'static str {
        match self {
            MsgType::UserCall => "USER_CALL",
            MsgType::DelRoom => "DEL_ROOM",
            MsgType::OppositeLeave => "OPPOSITE_LEAVE",
            MsgType::PushItems => "PUSH_ITEMS",
            MsgType::MerchantAdd => "MERCHANT_ADD",
            MsgType::UserAdd => "USER_ADD",
            MsgType::MerchantRejet => "MERCHANT_REJET",
        }
    }
...
pub mod merchant_notify_server {
    ...
    #[async_trait]
    pub trait MerchantNotify: Send + Sync + 'static {
        ///
    ...
    ///
    #[derive(Debug)]
    pub struct MerchantNotifyServer<T: MerchantNotify> {
        inner: Arc<T>,
    ...
    impl<T: MerchantNotify> MerchantNotifyServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: MerchantNotify> Clone for MerchantNotifyServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: MerchantNotify> tonic::server::NamedService for MerchantNotifyServer<T> {
        const NAME: &'static str = "bilibili.broadcast.message.mall.MerchantNotify";
    }
}
