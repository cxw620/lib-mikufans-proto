...
impl RecordType {
    ...
    pub fn as_str_name(&self) -> &'static str {
        match self {
            RecordType::All => "All",
            RecordType::Input => "input",
            RecordType::Output => "output",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
...
pub mod gallery_interface_server {
    ...
    #[async_trait]
    pub trait GalleryInterface: Send + Sync + 'static {
        ///
    ...
    ///
    #[derive(Debug)]
    pub struct GalleryInterfaceServer<T: GalleryInterface> {
        inner: Arc<T>,
    ...
    impl<T: GalleryInterface> GalleryInterfaceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: GalleryInterface> Clone for GalleryInterfaceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: GalleryInterface> tonic::server::NamedService for GalleryInterfaceServer<T> {
        const NAME: &'static str = "bilibili.pangu.gallery.v1.GalleryInterface";
    }
}
