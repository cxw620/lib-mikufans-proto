...
pub mod fission_server {
    ...
    #[async_trait]
    pub trait Fission: Send + Sync + 'static {
        ///
        async fn entrance(
            &self,
    ...
    ///
    #[derive(Debug)]
    pub struct FissionServer<T: Fission> {
        inner: Arc<T>,
    ...
    impl<T: Fission> FissionServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: Fission> Clone for FissionServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Fission> tonic::server::NamedService for FissionServer<T> {
        const NAME: &'static str = "bilibili.account.fission.v1.Fission";
    }
}
