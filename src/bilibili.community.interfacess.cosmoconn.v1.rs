...
impl Action {
    ...
    pub fn as_str_name(&self) -> &'static str {
        match self {
            ...
            Action::Thumb => "ACTION_THUMB",
            Action::CancelThumb => "ACTION_CANCEL_THUMB",
            Action::Fav => "ACTION_FAV",
            Action::CancelFav => "ACTION_CANCEL_FAV",
        }
    }
...
impl EntityBiz {
    ...
    pub fn as_str_name(&self) -> &'static str {
        match self {
            EntityBiz::Invalid => "ENTITY_BIZ_INVALID",
            EntityBiz::Video => "ENTITY_BIZ_VIDEO",
            EntityBiz::Opus => "ENTITY_BIZ_OPUS",
            EntityBiz::Dynamic => "ENTITY_BIZ_DYNAMIC",
            EntityBiz::Reply => "ENTITY_BIZ_REPLY",
            EntityBiz::Topic => "ENTITY_BIZ_TOPIC",
        }
    }
    /// Creates an enum from field names used in the ProtoBuf definition.
    pub fn from_str_name(value: &str) -> ::core::option::Option<Self> {
...
pub mod cosmo_interface_server {
    ...
    #[async_trait]
    pub trait CosmoInterface: Send + Sync + 'static {
        ///
        async fn ping(
            &self,
    ...
    ///
    #[derive(Debug)]
    pub struct CosmoInterfaceServer<T: CosmoInterface> {
        inner: Arc<T>,
    ...
    impl<T: CosmoInterface> CosmoInterfaceServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T: CosmoInterface> Clone for CosmoInterfaceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: CosmoInterface> tonic::server::NamedService for CosmoInterfaceServer<T> {
        const NAME: &'static str = "bilibili.community.interfacess.cosmoconn.v1.CosmoInterface";
    }
}
